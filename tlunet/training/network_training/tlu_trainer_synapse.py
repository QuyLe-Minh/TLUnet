...
import numpy as np
import torch
from tlunet.training.data_augmentation.data_augmentation_moreDA import get_moreDA_augmentation
from tlunet.training.loss_functions.deep_supervision import MultipleOutputLoss2
from tlunet.utilities.to_torch import maybe_to_torch, to_cuda
from tlunet.network_architecture.synapse.unetr_pp_synapse import TLUnet
from tlunet.network_architecture.initialization import InitWeights_He
from tlunet.network_architecture.neural_network import SegmentationNetwork
from tlunet.training.data_augmentation.default_data_augmentation import default_2D_augmentation_params, \
    get_patch_size, default_3D_augmentation_params
from tlunet.training.dataloading.dataset_loading import unpack_dataset
from tlunet.training.network_training.Trainer_synapse import Trainer_synapse
from tlunet.utilities.nd_softmax import softmax_helper
from sklearn.model_selection import KFold
from torch import nn
from torch.cuda.amp import autocast
from tlunet.training.learning_rate.poly_lr import poly_lr
from batchgenerators.utilities.file_and_folder_operations import *
from fvcore.nn import FlopCountAnalysis

class tlu_trainer_synapse(Trainer_synapse):
    """
    ...
    def initialize_network(self):
        ...
        self.network = TLUnet(self.num_classes, self.deep_supervision)   
    ...
    def applyWeight(self, model_weight):
        ...
        print("Apply weight successfully")
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        # print(self.num_classes, torch.unique(target))
        # weights = torch.tensor([1.] + [torch.sum((target == i).to(torch.float)) for i in range(1, self.num_classes)])
        # weights = (weights.sum() - 1)/weights
        # weights[weights == float("inf")] = 1
        # weights = (weights/torch.min(weights[1:])) ** 0.6

        # weights[0] = 1.
        # weights = to_cuda(weights)
        
        # self.loss.ce.weight = weights
        # self.loss.dc.weight = weights[1:]
